/**
 * Événements dataLayer (GTM), plan de taggage.
 * Fonctionne AVANT le chargement de GTM : la file est rejouée lorsque le conteneur se charge, après
 * consentement. Rien n'est perdu entre l'arrivée et l'accord, et rien n'est envoyé nulle part tant
 ...
 *  - souscription_ouverte  oui · non : tunnel fermé, tous les appels aboutissent à une fenêtre
 *                          d'attente, les taux d'avant et d'après ne mesurent pas la même chose ;
 *  - campagne_source,
 ...
 *             destination), émis par src/scripts/recherche/panneau.ts
 *  incidents  page_introuvable
 *  Le consentement est envoyé par scripts/consent.ts. Les quatre événements outil_* des anciens
 *  simulateurs, retirés le 14/09/2026, restent À RETIRER DE GA4 ET GTM.
 * JAMAIS DANS LE DATALAYER : aucun montant saisi dans un simulateur, aucune donnée personnelle ; le
 * panneau écarte les termes de recherche qui ressemblent à une adresse ou portent des chiffres.
 */
...
const init = () => {
  ...
  /* Les événements arrivent tout formés par un événement DOM plutôt qu'un import : aucun émetteur
     (panneau, consentement, fenêtre « bientôt ») n'a à connaître la mesure, ni l'inverse. Ce n'est
     PAS une précaution contre une double exécution, un module importé deux fois n'est exécuté
     qu'une fois. */
  ...
  if (document.body.dataset.pageType === '404') {
    /* Le référent SANS sa chaîne de requête ni son ancre, qui peuvent porter un identifiant ou un
       jeton : pour retrouver un lien mort, le site et la page suffisent. */
    let referent = '';
    try {
...
/** Profondeur de lecture aux quatre quarts de la page, une fois chacun : un événement par section
    tirait des dizaines d'événements par visite, de quoi noyer le reste et consommer le quota. */
